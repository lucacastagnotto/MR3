...
object mainApp {
  ...
  """val usage =
  Usage: Movie_recommender.jar [--cloud bool] [--arg2 num] filename
  """
  ...
  /*  if(args.length == 0) println(usage)
    ...
    def nextOption(map : OptionMap, list: List[String]) : OptionMap = {
      def isSwitch(s : String) = (s(0) == '-')                 //sSwitch simply checks for the first character being a dash
      list match {
        case Nil => map
        case "--cloud" :: value :: tail =>
          nextOption(map ++ Map("cloud" -> value.toBoolean), tail)
        case "--arg2" :: value :: tail =>
          nextOption(map ++ Map("arg2" -> value.toInt), tail)
    ...
    println(options)
    print("cloud:  " + options.getOrElse("cloud", "No such state"))
*/
   ...
   val t1 = System.nanoTime // execution duration
    // SparkSession
     ...
     // Initial variables
     //println("num partitions ds_full: " + ds_full)
     //def trans = ds_full.repartition(8)

     //val transactions = ds_full.map(row => (row(1),( row(0), row(3)))).groupByKey()//.map(t => t._2.map(_.toString.toInt))
     val transactions = ds_full.map(row => (row._2._1, row._1)).aggregateByKey(List[String]())((acc, x) => x :: acc,
                                                                                                    (acc1, acc2) => acc1 ::: acc2 )//.map(t => t._2.map(_.toString.toInt))     //lista film guardati per utente???
     transactions.collect().foreach(println)
     // ^ questa riga può essere migliorata perchè c'è molto shuffling, dobbiamo sostituire il groupByKey con un reduceByKey
     // quale era lo scopo di questa istruzione?

     println("num partitions: " + transactions.getNumPartitions)

     /*val totalTransactions = transactions.count().toInt
     val min_support_coef = 0.15
    ...
    // frequent_itemsets.map( p => spark_session.sparkContext.parallelize(p).saveAsTextFile("./outputFileProva"))
     spark_session.sparkContext.parallelize(frequent_itemsets).saveAsTextFile("./outputFileProva")
     bw.close()
 */
